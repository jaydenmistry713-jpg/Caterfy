...
- Orders dashboard with fixed-price and quote flows
  ...
  - Card payments use Stripe **Embedded Checkout** inline in the order dialog (destination charge to caterer's connected account); on completion Stripe redirects the top window to /order-status, which reconciles the payment. **Card-order side effects (confirmation emails, stock decrement, discount usage, first-order celebration) are deferred until payment succeeds** — see the "deferred side effects" note below; abandoning the inline checkout emails no one and consumes no stock
...
- ~~**Card payment status update (no-webhook approach)**~~ **DONE (July 2026)**: Card orders reconcile on the success redirect without needing a webhook.
  - The embedded session's `return_url` in `app/api/orders/route.ts` includes `&session_id={CHECKOUT_SESSION_ID}`
  - The `/order-status` page reconciles server-side when `session_id` is present (`reconcileCardPayment`): retrieves the Stripe session, and if `payment_status === 'paid'` transitions the order to `payment_status='paid'`, `status='accepted'` (auto-accept), `accepted_at`, `stripe_payment_intent_id` via a **compare-and-set** update (`.neq('payment_status','paid').select()`), then — only if that update actually flipped a row — calls `finalizeCardOrder()`
  - A standalone `/api/orders/verify-payment` route does the same for any client-side caller
  - The `checkout.session.completed` webhook case was **kept** (now also auto-accepts + finalizes) for resilience if a webhook is configured — it's idempotent with the redirect path via the same compare-and-set
- **Deferred card-order side effects (`lib/orders/finalize.ts`)** — because a card order isn't real until it's paid, `/api/orders` **skips** the confirmation emails (caterer notification + customer confirmation), stock decrement, discount-usage increment and first-order celebration for card-pending orders (`isCardPending`), and instead creates the order + Stripe session. `finalizeCardOrder(supabase, orderId)` runs that whole set **plus** the customer acceptance/review email exactly once at payment confirmation. It's invoked from all three reconcile paths (`/order-status`, `/api/orders/verify-payment`, webhook `checkout.session.completed`), each gated behind the compare-and-set so it can't double-fire. Non-card orders (offline / bank transfer / quote) are unchanged and still notify/decrement at creation. **Result:** starting and abandoning the inline checkout emails no one and consumes no stock/discount; an abandoned card order lingers as `pending`/`unpaid` until the 48h auto-cancel cron clears it.
